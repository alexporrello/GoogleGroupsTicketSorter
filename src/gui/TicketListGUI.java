...
import jm.JMButton;
import jm.JMPanel;
import reader.Ticket;
import scraper.GoogleGroupsScraper;

public class TicketListGUI extends JMPanel {
	...
	public TicketListGUI(JComponent content) {
	...
	public void addAllTickets() {
		...
		for (File f : GoogleGroupsScraper.SUPPORT_PAGES.listFiles()) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					Path dir = GoogleGroupsScraper.SUPPORT_PAGES.toPath();
	...
	public class TicketButton extends JMButton {
		...
		public TicketButton(File url) {
			...
			addActionListner(e-> {
				...
				content.add(new TicketGUI(new Ticket(url)), BorderLayout.CENTER);
				content.revalidate();
				content.repaint();
			});
		}
